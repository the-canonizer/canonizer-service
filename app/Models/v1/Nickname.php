...
class Nickname extends Model
{
    ...
    {
        return $this->hasMany(Camp::class, 'nick_name_id', 'nick_name_id');
    }

    public function supports()
    {
        return $this->hasMany(Support::class, 'nick_name_id', 'nick_name_id')->orderBy('support_order', 'ASC');
    }
    ...
    public function getDelegatedSupportCampList($namespace = 1, $id = 0, $filter = [])
    {
        $as_of_time = time();
        $as_of_clause = '';
        ...
        foreach ($results as $rs) {
            ...
            if ($rs->delegate_nick_name_id && $camp_num != 1) {
                ...
                if ($rs->title == '') {
                    $topicData = \App\Models\v1\Topic::where('topic_num', '=', $topic_num)->where('go_live_time', '<=', time())->latest('submit_time')->get();
