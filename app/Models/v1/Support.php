...
class Support extends Model
{
    ...
    {
        return $this->hasOne(Camp::class, 'camp_num', 'camp_num')->where('camp.objector_nick_id', '=', null)->where('camp.topic_num', $this->topic_num)->orderBy('camp.submit_time', 'DESC');
    }

    public function topic()
    {
        return $this->hasOne(Topic::class, 'topic_num', 'topic_num')->where('topic.objector_nick_id', '=', null)->where('topic.go_live_time', '<=', time())->orderBy('topic.submit_time', 'DESC');
    }
    ...
    public static function ifIamSingleSupporter($topic_num, $camp_num, $userNicknames)
    {
        $othersupports = [];
        $supportFlag = 1;
        if ($camp_num != 0) {
    ...
    public static function getAllSupporters($topic, $camp, $excludeNickID)
    {
        ...
        if (count($support) > 0 || count($support) > 0) {
