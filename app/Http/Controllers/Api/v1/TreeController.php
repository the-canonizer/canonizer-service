        ...
        try {
            ...
            if (in_array($algorithm, $algorithms) && ! ($fetchTopicHistory) && ! $commandStatus) {
                ...
                if ($isLastJobPending || $asOf == 'review') {
                    $tree = [TreeServiceFacade::getTopicTreeFromMysql($topicNumber, $algorithm, $asOfTime, $updateAll, $request)];
                } else {
                            ...
                            if ($asOfDate < $mongoTree[0]->as_of_date) {
                                ...
                                if ((! $mongoTree || ! count($mongoTree))) {
                                    ...
                                    if ($topicExistInMySql) {
                                        $mongoTree = [TreeServiceFacade::upsertTree($topicNumber, $algorithm, $asOfTime, $updateAll, $request)];
                                    }
                                }
                            }
                ...
                $tree = [TreeServiceFacade::getTopicTreeFromMysql($topicNumber, $algorithm, $asOfTime, $updateAll, $request, $fetchTopicHistory)];
            }
