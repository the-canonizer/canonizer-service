...
class TreeController extends Controller
{
    ...
    public function find(TreeStoreRequest $request)
    {
        try {
            ...
            // When true, scores are recomputed/served excluding bot (AI agent) users' support.
            $excludeBots = (bool) $request->input('exclude_bots', 0);
            // get the tree from mongoDb
            $start = microtime(true);

            // If the relevant bulk-generation command is running, fetch tree from MySQL.
            // The bot-excluded variant is generated by a separate command, so guard on the
            // matching signature to keep the two caches isolated.
            $commandSignature = $excludeBots ? "tree:bot:all" : "tree:all";
            $commandStatement = "php artisan " . $commandSignature;
            ...
            if (in_array($algorithm, $algorithms) && !($fetchTopicHistory) && !$commandStatus) {
                ...
                if ($isLastJobPending || $asOf == "review") {
                    $tree = array(TreeService::getTopicTreeFromMysql($topicNumber, $algorithm, $asOfTime, $updateAll, $request, $fetchTopicHistory, $excludeBots));
                } else {
                            ...
                            if ($asOfDate < $mongoTree[0]->as_of_date) {
                                ...
                                if ((!$mongoTree || !count($mongoTree))) {
                                    ...
                                    if ($topicExistInMySql) {
                                        $mongoTree = array(TreeService::upsertTree($topicNumber, $algorithm, $asOfTime, $updateAll, $request, $excludeBots));
                                    }
                                }
                            }

                            if ($mongoTree && count($mongoTree)) {
                                // Pick the human-only variant fields when bot exclusion is requested.
                                $treeStructure = $excludeBots
                                    ? ($mongoTree[0]['tree_structure_excl_bots'] ?? null)
                                    : $mongoTree[0]['tree_structure'];

                                // Human-only variant not generated for this cached doc yet -> compute + cache it once.
                                if ($excludeBots && empty($treeStructure)) {
                                    $botRecord = TreeService::upsertTree($topicNumber, $algorithm, $asOfTime, $updateAll, $request, true);
                                    $treeStructure = (is_array($botRecord) || $botRecord instanceof \ArrayAccess) ? ($botRecord['tree_structure_excl_bots'] ?? null) : null;
                                }

                                if (empty($treeStructure)) {
                                    $tree = array(TreeService::getTopicTreeFromMysql($topicNumber, $algorithm, $asOfTime, $updateAll, $request, $fetchTopicHistory, $excludeBots));
                                } else {
                                    $tree = collect([$treeStructure]);
