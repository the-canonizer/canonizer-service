...
class TreeGetApiTest extends TestCase
{
    ...
    {
        $response = $this->postJson($this->apiURl, ['topic_num' => '', 'asofdate' => '', 'algorithm' => '', 'update_all' => '']);
        $this->assertEquals(422, $response->status());
    }
    ...
    {
        $response = $this->postJson($this->apiURl, ['topic_num' => 238, 'asofdate' => time(), 'algorithm' => 'blind_popularity', 'update_all' => 0, 'view' => '$QWZhYWNiVmhtMTE2ekt3Vg$iIc0UGbTCDIgCXXwpHnzXA']);
        $this->assertEquals(200, $response->status());
    }

    public function testWithCorrectValuesForValidResponseStructure()
    {
        $response = $this->postJson($this->apiURl, ['topic_num' => 238, 'asofdate' => time(), 'algorithm' => 'blind_popularity', 'update_all' => 0]);
